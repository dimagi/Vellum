...
describe("The SaveToCase module", function() {
    ...
    describe("attribute-path reference validation", function () {
        ...
        // Suffix-level tests drive validation by populating `mug.p` directly
        // and asserting against `mug.messages` — not through DOM events,
        // because the rich-text xpath field owns its own state and
        // `.val().trigger('change')` on the underlying $el bypasses the
        // editor's change pipeline. The logic.js `_addReferences` path is
        // what writes "Unknown question: X" into mug.messages; widgets.js
        // shares the same rule and doesn't need its own DOM-wired test.
        function hasUnknownRef(mug) {
            var found = false;
            mug.messages.each(function (m) {
                if (m.message && m.message.indexOf("Unknown question") !== -1) {
                    found = true;
                }
            });
            return found;
        ...
        it("should accept valid SaveToCase attribute paths", function () {
            util.loadXML("");
            var parent = util.addQuestion("SaveToCase", "parent_stc", {
                case_id: "uuid()", useCreate: true,
                case_type: "p", caseName: "/data/name",
            });
            var user = util.addQuestion("Text", "user_ref", {
                calculateAttr: parent.absolutePath + "/case/@case_id",
            });
            assert.notOk(
                hasUnknownRef(user),
                "/case/@case_id is a real attribute — must not flag"
            );
        });

        it("should flag a typo in attribute name under a SaveToCase mug", function () {
            util.loadXML("");
            var parent = util.addQuestion("SaveToCase", "parent_stc", {
                case_id: "uuid()", useCreate: true,
                case_type: "p", caseName: "/data/name",
            ...
            var user = util.addQuestion("Text", "user_ref", {
                calculateAttr: parent.absolutePath + "/case/@rabbish",
            });
            assert.ok(
                hasUnknownRef(user),
                "@rabbish is not a real SaveToCase output attr — must flag"
            );
        });

        it("should flag /case/update/<k> — not whitelisted", function () {
            // `/case/update/*` binds are emitted output, not expected
            // cross-references. Even a "valid" key like `age` is rejected
            // because nothing in real forms reads back from the update
            // subtree; this preserves typo detection for any accidental
            // mis-navigation.
            util.loadXML("");
            var parent = util.addQuestion("SaveToCase", "parent_stc", {
                case_id: "/data/some_id",
                useUpdate: true,
            });
            parent.p.updateProperty = { age: { calculate: "/data/age" } };

            var ref = util.addQuestion("Text", "ref", {
                calculateAttr: parent.absolutePath + "/case/update/age",
            });
            assert.ok(
                hasUnknownRef(ref),
                "/case/update/* is not part of the whitelist — must flag"
            );
        });

        it("should still flag arbitrary sub-paths under non-SaveToCase mugs", function () {
            // Plain Text mug doesn't opt into `isValidSubPath`, so
            // anything below its path must be rejected — walk-up tolerance
            // is per-mug-type, not global.
            util.loadXML("");
            var text = util.addQuestion("Text", "question");
            var referrer = util.addQuestion("Text", "referrer", {
                calculateAttr: text.absolutePath + "/bogus/thing",
            });
            assert.ok(
                hasUnknownRef(referrer),
                "Text mug has no output subtree — any sub-path must be flagged"
            );
        });
    });
});
